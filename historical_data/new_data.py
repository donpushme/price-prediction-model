...
closes = np.array(all_closes, dtype=np.float32)[:60]
times = np.array(all_times, dtype=np.int64)[:60]

# Save to npy
np.save("./historical_data/new_data.npy", {"close": closes, "time": times})

# Save to json
with open("./historical_data/new_data.json", "w", encoding="utf-8") as f:
    json.dump({
